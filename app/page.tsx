...
export default function HomePage() {
  ...
  const parallaxY = useTransform(scrollYProgress, [0, 1], [0, 80]); // moves glow gently as you scroll

  useEffect(() => {
    // Poll live once at mount
    fetch("/api/live")
      .then((r) => r.json())
      .then((d: LiveData) => {
        setLive(d);
    ...
    // Fetch latest videos for the rail
    ...
    <div>
      ...
      <section className="container py-16" ref={heroRef}>
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div variants={stagger} initial="initial" whileInView="animate" viewport={{ once: true }}>
            ...
            >
              ...
              <a href="/prayer" className="btn-ghost">
                Prayer Request
              </a>
            </motion.div>
            ...
            {!live?.live && (
              ...
              >
                Next service: <strong>{nextSvc.toLocaleString()}</strong> · {countdown.days}d {countdown.hours}h{" "}
                {countdown.minutes}m
              </motion.p>
            )}
          </motion.div>

          {/* Visual card with gentle parallax glow */}
          <motion.div
            {...fadeUp}
            transition={{ ...fadeUp.transition, delay: 0.12 }}
            className="card overflow-hidden relative"
          >
            {/* Subtle moving glow behind the image */}
            <motion.div
              style={{ y: parallaxY }}
            ...
            <div className="aspect-video relative">
              <img src={heroSrc} alt="CLM Preview" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent" />
              <div className="absolute inset-0 flex items-end p-4 md:p-6">
                <div className="flex flex-wrap gap-3">
                  <a href={watchHref} target="_blank" rel="noreferrer" className="btn-primary">
                    Watch on YouTube
                  </a>
        ...
        >
          {[
            {
              title: "Give Online",
              desc: "Partner with CLM — secure giving.",
              href: "/give",
            ...
            {
              title: "Plan a Visit",
              desc: "See what’s happening this month.",
              href: "/events",
      ...
      <section className="container pb-16 grid md:grid-cols-2 gap-6">
        ...
        >
          ...
          <p className="text-white/70 mt-2">{nextSvc.toLocaleString()} · Birmingham, AL</p>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <a
              ...
              )}&start=${encodeURIComponent(nextSvc.toISOString())}&end=${encodeURIComponent(
                new Date(nextSvc.getTime() + 2 * 60 * 60 * 1000).toISOString()
