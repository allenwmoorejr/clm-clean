...
export default function HomeClient() {
  ...
  const [heroSrc, setHeroSrc] = useState("/pastor-hero.jpg"); // curated default
  ...
  useEffect(() => { setEnableTilt(window.matchMedia?.("(pointer: fine)").matches ?? false); }, []);
  function onMove(e: React.MouseEvent<HTMLDivElement>) {
    if (!enableTilt) return;
    const r = e.currentTarget.getBoundingClientRect();
    const px = (e.clientX - r.left) / r.width; const py = (e.clientY - r.top) / r.height;
    const max = 6; setTilt({ x: (py - 0.5) * -2 * max, y: (px - 0.5) * 2 * max });
  }
  const resetTilt = () => setTilt({ x: 0, y: 0 });

  useEffect(() => {
    fetch("/api/live").then(r=>r.json()).then((d:LiveData)=>{
      setLive(d);
      if (d?.live && d.videoId){
    ...
    fetch("/api/videos?limit=12").then(r=>r.json()).then(d=>setVideos(d.items||[])).catch(()=>setVideos([])).finally(()=>setLoadingVideos(false));
    ...
    <div>
      ...
      <section className="container py-12 sm:py-16 relative" ref={heroRef}>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <motion.div variants={stagger} initial="initial" whileInView="animate" viewport={{ once: true }}>
            ...
            {!live?.live && (
              <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.34 }} className="mt-5 text-white/70 text-sm">
                Next service: <strong>{nextServiceDate().toLocaleString()}</strong> · {countdown.days}d {countdown.hours}h {countdown.minutes}m
              </motion.p>
            )}
          </motion.div>
          ...
          <motion.div {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.12 }}
            ...
            onMouseMove={onMove} onMouseLeave={resetTilt}
            ...
            <motion.div style={{ transform:`rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)`, transformStyle:"preserve-3d" }}
              transition={{ type:"spring", stiffness:200, damping:20 }} className="aspect-video relative">
              <motion.img key={heroSrc} src={heroSrc} alt="CLM Preview" className="w-full h-full object-cover"
                initial={{ scale: 1.02 }} animate={{ scale: 1.08 }}
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-10">
          {[
            ...
            <motion.a key={c.title} href={c.href}
              ...
              transition={{ duration: 0.5, delay: i*0.06 }} viewport={{ once:true, amount:0.3 }}>
              <div className="flex items-center gap-3">
                {c.icon}<h3 className="text-xl font-semibold">{c.title}</h3>
              </div>
      ...
      <section className="container pb-6">
        ...
        <motion.div
          ...
          onTouchStart={()=>hoverRef.current=true} onTouchEnd={()=>hoverRef.current=false}
        >
          <div className="flex gap-4 min-w-full pr-2">
            ...
            {videos.map((v)=>(
              <motion.a key={v.id} href={`https://www.youtube.com/watch?v=${v.id}`} target="_blank" rel="noreferrer"
                className="card overflow-hidden w-[76vw] sm:w-[290px] shrink-0 group snap-start"
