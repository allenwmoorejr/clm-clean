...
export default function HomeClient() {
  ...
  const [heroSrc, setHeroSrc] = useState("/pastor-hero.jpg"); // curated default when not live
  ...
  // Tilt (desktop/fine pointer only)
  ...
  // Load live status + hero poster & videos
  useEffect(() => {
    fetch("/api/live")
      .then((r) => r.json())
      .then((d: LiveData) => {
    ...
    const t = setInterval(() => setCountdown(countdownParts(nextServiceDate())), 30000);
    ...
    <div>
      ...
      <section className="container py-12 sm:py-16 relative" ref={heroRef}>
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <motion.div variants={stagger} initial="initial" whileInView="animate" viewport={{ once: true }}>
            ...
            >
              <span className="badge">
                <CalendarDays size={14} /> Sundays · 10:00am CT
              </span>
              <span className="badge">
                <Radio size={14} /> WJLD 1400 AM · 1–2pm
              </span>
              <span className="badge">
                <MapPin size={14} /> Birmingham, AL
              </span>
            </motion.div>
            ...
            >
              <Link href="/watch" className="btn-primary tap-target">
                <PlayCircle size={18} /> Watch Live
              </Link>
              <Link href="/sermons" className="btn-ghost tap-target">
                Sermon Library
              </Link>
              <a href="/prayer" className="btn-ghost tap-target">
                Prayer Request
              </a>
            </motion.div>
            ...
            {!live?.live && (
              ...
              >
                Next service: <strong>{nextServiceDate().toLocaleString()}</strong> · {countdown.days}d {countdown.hours}h{" "}
                {countdown.minutes}m
              </motion.p>
            )}
          </motion.div>

          {/* Visual: Ken Burns + Tilt + Glow + top-left CTAs */}
          <motion.div
            {...fadeUp}
              ...
              <div className="hidden sm:flex absolute inset-0 items-start justify-start">
                <div className="p-4 md:p-6" style={{ transform: "translateZ(20px)" }}>
                  <div className="flex flex-wrap gap-3">
                    <a href={watchHref} target="_blank" rel="noreferrer" className="btn-primary tap-target">
                      Watch on YouTube
                    </a>
                    <Link href="/watch" className="btn-ghost tap-target">
                      Open Watch Page
                    </Link>
                  </div>
                </div>
          ...
          <div className="sm:hidden -mt-6 flex flex-wrap gap-3">
            <a href={watchHref} target="_blank" rel="noreferrer" className="btn-primary tap-target">
              Watch on YouTube
            </a>
            <Link href="/watch" className="btn-ghost tap-target">
      ...
      {/* Latest Sermons rail (auto-scroll, pause on hover/touch) */}
      <section className="container pb-6">
        <div className="flex items-center justify-between">
          <h2 style={{ fontSize: "var(--h2)" }}>Latest Sermons</h2>
          <Link href="/sermons" className="text-white/70 hover:text-white text-sm">
            View all
          </Link>
        </div>
        ...
        >
          <div className="flex gap-4 min-w-full pr-2">
            ...
            {!loadingVideos && videos.length === 0 && (
              <div className="text-white/60 text-sm">No videos found yet.</div>
            )}
            {videos.map((v) => (
              <motion.a
                key={v.id}
                ...
                target="_blank"
                rel="noreferrer"
                className="card overflow-hidden w-[82vw] sm:w-[300px] md:w-[320px] shrink-0 group snap-start"
              ...
              >
                <div className="aspect-video overflow-hidden relative">
                  <NextImage
                    ...
                    sizes="(max-width: 640px) 82vw, (max-width: 768px) 300px, 320px"
                    onError={(e) => {
                      // Fallback to hqdefault if thumb url fails
                      const img = e.target as HTMLImageElement;
                      img?.setAttribute?.("src", `https://i.ytimg.com/vi/${v.id}/hqdefault.jpg`);
                    }}
                  />
                </div>
                <div className="p-4">
      ...
      <section className="container pb-16 grid md:grid-cols-2 gap-6">
        <motion.div {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.05 }} className="card p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <Link href="/events" className="btn-ghost tap-target">
              See all events
            </Link>
          </div>
        </motion.div>
